...
export function UniversityTableFilter({
  onChange,
}: UniversityTableFilterProps) {
  type FilterField = Exclude<keyof University, "Name" | "Initial">;

  const [currentKey, setCurrentKey] = useState<FilterField | null>(
    null,
  );
  ...
  useEffect(() => {
    setCurrentValue(
      currentKey ? options[currentKey].values[0] : null,
    );
  }, [currentKey]);

  useEffect(() => {
    onChange({
      newFilter: (university) =>
        !currentKey ||
        !currentValue ||
        university[currentKey] === currentValue,
    });
  }, [currentValue]);
    ...
    <>
      <div className="flex flex-col">
        <div className="flex flex-wrap justify-center items-center gap-2">
          <Icon
            icon={faFilter}
            className={currentKey ? "text-purple-500" : "gray-text"}
          />
          {currentKey ? (
            <span className="select-none">Filtrar por</span>
          ) : null}
          <Select
            onChange={({ target }) =>
              setCurrentKey((target.value as FilterField) || null)
            }
          >
            {currentKey ? (
          ...
          {currentKey ? (
            ...
            >
              ...
              {options[currentKey].values.map((_value, index) => {
                const value = _value;
                const label =
                  options[currentKey].formatValue?.(_value) ?? _value;
                return (
